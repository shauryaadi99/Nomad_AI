...
const CreateTripsPage = () => {
  ...
  const OnGenerateTrips = async () => {
    ...
    if (!user || !user.email) {
      toast.custom(() => (
 <div
    className="flex items-center justify-center w-full max-w-xs p-3 bg-black text-[#FFD700] font-semibold rounded-md shadow-lg"
    style={{ zIndex: 9999 }}
 ...
 <div
  ...
  >          ⚠️ You can only plan trips for up to 5 days!
    ...
    setLoaderMessage(
      "Unlocking Experiences & Designing Your Perfect Journey..."
    );

    // Prepare the final prompt for the AI
    ...
    try {
      ...
      setLoading(false);
      setLoaderMessage(""); // Clear the loader message after completion
    }
  };
  ...
  return (
    <div className="w-full min-h-screen bg-gray-900 text-white px-5 md:px-20 lg:px-32 xl:px-40 py-16">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Loader with catchy message */}
        {loaderMessage && (
          <div className="fixed top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-800 bg-opacity-95 px-8 py-6 rounded-2xl shadow-[0_0_20px_rgba(255,215,0,0.6)] backdrop-blur-md text-center text-white animate-fadeIn scale-100 transition-transform sm:px-6 sm:py-4 sm:text-sm sm:rounded-lg">
            <h2 className="text-2xl font-extrabold bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-300 bg-clip-text text-transparent sm:text-lg">
              {loaderMessage}
            </h2>
            <AiOutlineLoading3Quarters className="h-12 w-12 animate-spin mt-4 text-yellow-300 sm:h-8 sm:w-8" />
          </div>
        )}
      </div>
